...
def sma_forecast(lens, data, far):
    cali = sma(lens, data)
    set = data[(-lens + 1):] + [cali[-1]]
    for i in range(lens - 1, far):
        a = 0
        for j in range(1, lens + 1):
            a += set[i - j + 1]
        a = a / lens
...
if __name__ == '__main__':
    ...
    for i in range(2, len(series)):
        SMA_1 = sma_forecast(i, series,30)
        SMA_1.pop()
        temp.append(mse(SMA_1, series, i))
